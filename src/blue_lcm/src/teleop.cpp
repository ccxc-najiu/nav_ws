   ...
   a_scale_(2.0)
 {
   ...
   // 从参数服务器中调取参数scale_angular给第二个a_scale_，如果没有，将第三个参数值（即默认值）给第二个参数
 ...
 void Teleop_car::keyLoop()
 {
   ...
   for(;;)
   {
     ...
     twistStamped.twist.angular.z = a_scale_*angular_;
     twistStamped.twist.linear.x = l_scale_*linear_;
     if(dirty ==true)
     {
