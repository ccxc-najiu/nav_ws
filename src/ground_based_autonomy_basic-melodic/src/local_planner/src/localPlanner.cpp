...
// 接收激光里程计信息，并由外参计算机器人机体中心的位置姿态(全局)
...
int main(int argc, char **argv)
{
  ...
  nhPrivate.getParam("terrainVoxelSize", terrainVoxelSize); //  地形网格大小
  ...
  nhPrivate.getParam("vehicleRadis",vehicleRadis);
    ...
    if (newLaserCloud || newTerrainCloud)
    {
      ...
      for (int i = 0; i < plannerCloudSize; i++)
      {
        ...
        if (dis < adjacentRange && ((point.z > minRelZ && point.z < maxRelZ) || useTerrainAnalysis))
        {
          ...
          for(int i=-1;i<2;i++){
            for(int j=-1;j<2;j++){
                pointAdd.x=point.x+i*vehicleRadis;
                pointAdd.y=point.y+j*vehicleRadis;
                pointAdd.z=point.z;
                pointAdd.intensity=point.intensity;
                plannerCloudCrop->push_back(pointAdd);
      ...
      //  筛选机器人阈值内的边界点，并将该边界点转为当前帧下机体系的坐标
      int boundaryCloudSize = boundaryCloud->points.size();
      for (int i = 0; i < boundaryCloudSize; i++)
      {
        point.x = ((boundaryCloud->points[i].x - vehicleX) * cosVehicleYaw + (boundaryCloud->points[i].y - vehicleY) * sinVehicleYaw) / pathScale;
        point.y = (-(boundaryCloud->points[i].x - vehicleX) * sinVehicleYaw + (boundaryCloud->points[i].y - vehicleY) * cosVehicleYaw) / pathScale;
      ...
      //  筛选机器人周围阈值内的障碍物的点，并将该障碍物点转为当前帧下机体系的坐标
      int addedObstaclesSize = addedObstacles->points.size();
      for (int i = 0; i < addedObstaclesSize; i++)
      {
        point.x = ((addedObstacles->points[i].x - vehicleX) * cosVehicleYaw + (addedObstacles->points[i].y - vehicleY) * sinVehicleYaw) / pathScale;
        point.y = (-(addedObstacles->points[i].x - vehicleX) * sinVehicleYaw + (addedObstacles->points[i].y - vehicleY) * cosVehicleYaw) / pathScale;
      ...
      while (pathScale >= minPathScale && pathRange >= minPathRange)
      {
        ...
        for (int i = 0; i < 36 * pathNum; i++)
        {
          ...
          if (clearPathList[i] < pointPerPathThre)
          {
            ...
            // 目标点与当前状态的方向差减去本条路径终点与与当前状态的方向差
            ...
            float rotDirW;
            // if (rotDir < 18) rotDirW = fabs(fabs(rotDir - 11) + 1);
            // else rotDirW = fabs(fabs(rotDir - 29) + 1);
