...
void goal_callback(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
  ...
  if (msg->pose.position.z >= 0)
  {
    ...
    waypointMsgs.point.x = point.x;
    waypointMsgs.point.y = point.y;
    waypointMsgs.point.z = point.z;
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber subWaypoint = nh.subscribe<geometry_msgs::PoseStamped>("/move_base_simple/goal", 10, goal_callback);
